...
class HardwarePipelineClient:
    ...
    """
    def __init__(self, state: ControlState, host: str = "192.168.0.108", port: int = 8000):
        self.state = state
        self.host = host
        self.port = port
        self._running = True
            ...
            try:
                ...
                while self._running:
                    ...
                    telemetry = json.dumps({
                        ...
                        "pwm1": self.state.pwm1,
                        "pwm2": self.state.pwm2,
                        "timestamp": time()
                    })
                    try:
                        ws.send(telemetry)
...
def get_ahrs(TR, dt):
    ...
    # Pitch is obtained directly from the pitch encoder (encoder2)
    # raw_enc2 = -170 corresponds to pitch = 0 deg
    # raw_enc2 = 185 corresponds to pitch = +80 deg (mechanical stop)
    # scale = 80.0 / (185 - (-170)) = 80.0 / 355.0
    raw_enc2 = TR.encoder.encoder2
    pitch_deg = (raw_enc2 + 170.0) * (80.0 / 355.0)
    pitch = np.radians(pitch_deg)
    
    # Yaw is obtained directly from magnetometer (IMU)
    mag = TR.imu.magnetic
    yaw = normalize_angle(np.arctan2(mag[1], mag[0]))
...
def main():
    ...
    try:
        ...
        roll, init_pitch, init_yaw = get_ahrs(TR, dt_const)
        dt = TR.update_readings()

        # Startup offsets to define initial position as (0, 0)
        initial_pitch = init_pitch
        initial_yaw = init_yaw

        theta_prev = initial_yaw
        phi_prev = initial_pitch
        eIR = np.zeros(3)

        # Set initial target and telemetry angles to 0.0
        ...
        # Arm and start the motors (transition from standby to run state)
        TR.motors.runM0()
        TR.motors.runM1()
        ...
        while True:
            ...
            # Update telemetry state (relative to startup position) for sending to laptop server
            state.pitch_deg = float(np.clip(np.degrees(normalize_angle(pitch - initial_pitch)), -80.0, 80.0))
            state.yaw_deg = float(np.degrees(normalize_angle(yaw - initial_yaw)))
            state.encoder1 = int(TR.encoder.encoder1)

            # Reference generator using absolute angles by shifting relative target
            ref_p, ref_p_dot, ref_p_ddot, ref_y, ref_y_dot, ref_y_ddot = compute_reference(
                t_now,
                yaw_ref_deg=state.target_yaw_deg + np.degrees(initial_yaw),
                pitch_ref_deg=state.target_pitch_deg + np.degrees(initial_pitch),
                yaw_mode='fixed',
                pitch_mode='fixed'
            )

            # measured states mapped to model coordinates (absolute)
            theta_meas = yaw
            phi_meas = pitch
            ...
            if not state.active:
                ...
                # Map [-3500, 3500] RPM range to standard [1000, 2000] us ESC PWM range
                state.pwm1 = int(1500 + (m0_speed / 3500.0) * 500)
                state.pwm2 = int(1500 + (-m1_speed / 3500.0) * 500)
