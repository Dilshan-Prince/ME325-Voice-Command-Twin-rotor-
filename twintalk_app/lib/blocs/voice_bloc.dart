...
class VoiceManualCommandSubmitted extends VoiceEvent {
  final String transcript;
  VoiceManualCommandSubmitted(this.transcript);
...
class VoiceBloc extends Bloc<VoiceEvent, VoiceState> {
  ...
  VoiceBloc(this._whisper, this._agent) : super(VoiceIdle()) {
    ...
    on<VoiceManualCommandSubmitted>(_onManualCommand);
  }
  ...
  Future<void> _onSend(VoiceSendToAgentRequested event, Emitter<VoiceState> emit) async {
    ...
    try {
      final traj = await _agent.sendVoiceCommand(current.transcript);
  ...
  Future<void> _onManualCommand(VoiceManualCommandSubmitted event, Emitter<VoiceState> emit) async {
    emit(VoiceParsing(event.transcript));
    try {
      final intent = await _agent.parseIntent(event.transcript);
      emit(VoiceParsed(transcript: event.transcript, intent: intent));
    } catch (e) {
      emit(VoiceError(e.toString()));
    }
  }
