                    ...
                    if (history.length > 2) ...[
                      Builder(
                        builder: (context) {
                          double minY = -10.0;
                          double maxY = 45.0;
                          final pitches = history.map((e) => e.pitchDeg).toList();
                          if (pitches.isNotEmpty) {
                            double minP = pitches.reduce((a, b) => a < b ? a : b);
                            double maxP = pitches.reduce((a, b) => a > b ? a : b);
                            if (_targetPitch < minP) minP = _targetPitch;
                            if (_targetPitch > maxP) maxP = _targetPitch;
                            if (maxP - minP < 10.0) {
                              minY = minP - 5.0;
                              maxY = maxP + 5.0;
                            } else {
