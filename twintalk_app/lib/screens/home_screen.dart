...
class _HomeScreenState extends State<HomeScreen> {
  final _hostController = TextEditingController(text: '192.168.0.108');
  ...
  void _connect(BuildContext context) {
    ...
    // Update AgentOrchestrator dynamically to match connection host/port
    context.read<AgentOrchestrator>().backendHost = host;
    context.read<AgentOrchestrator>().backendPort = port;
                 ...
                 TTCard(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text('Laptop Server',
                          style: AppTheme.labelSmall.copyWith(color: AppTheme.primaryLight, fontSize: 11)),
                      const SizedBox(height: 8),
                      _field('Laptop IP Address', _hostController,
                          hint: '192.168.0.108'),
                      const SizedBox(height: 8),
                      _field('Laptop Port', _portController,
                          hint: '8000', keyboard: TextInputType.number),
                    ],
                  ),
                ),
                const SizedBox(height: 16),
                ...
                BlocBuilder<ConnectionBloc, ConnectionState>(
                  builder: (context, state) {
                    ...
                    return ElevatedButton.icon(
                      ...
                      label: Text(
                          isLoading ? 'Connecting…' : 'Connect to Devices'),
                    );
                  },
                ),
