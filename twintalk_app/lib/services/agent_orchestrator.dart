...
class AgentOrchestrator {
  ...
  /// IP of the laptop running LangGraph. Configured at runtime.
  String backendHost = '192.168.0.108';
  int backendPort = 8000;
  String get _base => 'http://$backendHost:$backendPort';
  ...
  ParsedIntent _mockIntent(String text) {
    ...
    final pitchMatch = RegExp(r'pitch\s+(?:to\s+)?(-?\d+(?:\.\d+)?)').firstMatch(text.toLowerCase());
    final yawMatch = RegExp(r'yaw\s+(?:to\s+)?(-?\d+(?:\.\d+)?)').firstMatch(text.toLowerCase());
    final parsedPitch = pitchMatch != null ? double.tryParse(pitchMatch.group(1)!) : 30.0;
    final pitch = parsedPitch?.clamp(-80.0, 80.0);
  ...
  // ── Send Voice Command (calls backend simulation execution) ────
  /// Sends the voice transcript directly to the FastAPI server command endpoint.
  /// This endpoint parses the command, runs the MeshCat simulation, and returns
  /// once the simulation is completed.
  Future<TrajectoryCommand> sendVoiceCommand(String transcript) async {
    _log.i('Sending voice command to agent: "$transcript"');
    try {
      final resp = await http
          .post(
      ...
      if (resp.statusCode == 200) {
        ...
        _log.i('Backend simulation complete. Parsed data: $parsedData');
        return TrajectoryCommand(
          targetPitchDeg: (parsedData['pitch'] as num).toDouble(),
          targetYawDeg: (parsedData['yaw'] as num).toDouble(),
          durationSec: 3.0,
        ...
        _log.e('Simulation command error ${resp.statusCode}: ${resp.body}');
        throw Exception('Simulation command error: ${resp.statusCode}');
      }
    } on Exception catch (e) {
      _log.w('Using mock/regex trajectory fallback (backend unreachable): $e');
      final intent = _mockIntent(transcript);
      return intent.toTrajectoryCommand();
    }
  }

  // ── Authorize Movement ─────────────────────────────────────────
  /// Sends the human-in-the-loop authorization status (Approve/Cancel)
  /// back to the Laptop Server.
  Future<void> authorizeMovement(bool approved) async {
    _log.i('Sending safety gate authorization: approved=$approved');
    try {
      final resp = await http
          .post(
            Uri.parse('$_base/api/authorize'),
            headers: {'Content-Type': 'application/json'},
      ...
      if (resp.statusCode == 200) {
        _log.i('Authorization successfully dispatched: ${resp.body}');
      } else {
        _log.e('Authorization failed ${resp.statusCode}: ${resp.body}');
      }
    } on Exception catch (e) {
      _log.e('Authorization communication error: $e');
    }
  }
}
