...
/// Manages the WebSocket connection to the Laptop control server.
/// The server runs a Python asyncio WebSocket server on port 8000 that:
...
class RotorConnectionService {
  ...
  // Temporary — for testing without Pi hardware
  static const String _backendHost = '192.168.0.108';  // laptop or Pi
  static const int _backendPort = 8000;

  // ── Connect ────────────────────────────────────────────────────
  Future<void> connect({String host = _backendHost, int port = _backendPort}) async {
    ...
    try {
      ...
      _log.i('Connected to Laptop Server @ $host:$port');
  ...
  /// Send an approved trajectory to the Laptop Server for simulation execution.
